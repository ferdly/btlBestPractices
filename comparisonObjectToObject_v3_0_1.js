...
async function testCollectionReportDisplay(testCollection = [], displayParamObject = {}) {
    ...
    let columnHeaderBlockHeader = 'SUBHEADER: some useful identifying attributes'
    ...
    testCollection.forEach(element => {
        ...
        if (element.assertion === 'COLUMNHEADER') { 
            let columnHeaderBufferLength = element.paddedString.length
            columnHeaderBuffer = headerBuffer.substr(0,columnHeaderBufferLength) 
            columnHeaderBlock += deliverRowByMechanism(element, 'PARAM-OBJECT-PENDING') + '' 
        }
