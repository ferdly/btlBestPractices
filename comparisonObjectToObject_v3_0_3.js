...
async function overrideWithGoldDefaults(paramObject = {}) {
    ...
    // paramObjectDefault.displayParamObject.develLogKeyArray.push()
...
async function comparisonWixToWixRecords(wixRecordA = {}, wixRecordB = {}, paramObject = {}) {
    ...
    console.warn(`≈416≈ testCollection: [${testCollection}] [instantiated]`)
    ...
    // let trimmedElement = JSON.parse(JSON.stringify(testCollection[0]))
    // delete trimmedElement.notes
    console.warn(`≈440≈ testCollection[0]: [COLUMNHEADER manual] [all instantiated attributes]`)
    // console.dir(trimmedElement)
    let keyShowArray = []
    // let keyShowArray = ['Chester']
    let lineObject = {}
    lineObject.logLine = '440Z'
    ...
    console.warn(`≈455≈ testCollection <≈664≈ForEach>[instantiate by keyArrayKeyArray]`)
    ...
    console.warn(`≈455≈ testCollection </≈585≈ForEach>[instantiate by keyArrayKeyArray]`)
    
    console.warn(`≈461≈ testCollection <≈ZZZ≈ForEach>[instantiate by ObjectA-ObjectB]`)
    byTestCollectionObjectAObjectB(wixRecordA, wixRecordB, testCollection, paramObject)
    console.warn(`≈461≈ testCollection </≈585≈ForEach>[instantiate by ObjectA-ObjectB]`)
...
function byTestCollectionObjectAObjectB(objectA = {}, objectB = {}, testCollection = [], paramObject = {}) {
    ...
    for (let index = 0; index < testCollection.length; index++) {
        ...
        if (paramObject.displayParamObject.develLogKeyArray.includes(testUnit.key)) {
            console.warn(`≈585≈ byTestUnitObjectAObjectB(): [After SWITCH-ed Singletons] key: ${testUnit.key}`)
...
function testCollectionAppend(testCollection = [], assertion = 'OTHER', keyArray = [], paramObject = {}) {
    for (let index = 0; index < keyArray.length; index++) {
        ...
        testCollection.push(testUnitThis)
        // ø </UnitTest>
        if (paramObject.displayParamObject.develLogKeyArray.includes(testUnitThis.key)) {
            console.warn(`≈664 testCollectionAppend(): key: ${testUnitThis.key}`)
            let trimmedElement = JSON.parse(JSON.stringify(testUnitThis))
            delete trimmedElement.notes
            // delete trimmedElement.response              //NA
            // delete trimmedElement.keyCountA             //NA
            // delete trimmedElement.keyCountB             //NA
...
async function testCollectionReportStringPrep(testCollection = [], displayParamObject = {}) {
    ...
    for (let index = 0; index < testCollection.length; index++) {
        ...
        console.warn(`≈1042≈ testCollectionReportStringPrep <≈ZZZ≈ForEach => composeReportStringDisplay()>`)
...
function composeCardinalityString(element = {}, displayParamObject = {}) {
    if (displayParamObject.develLogKeyArray.includes(element.key)) {
        console.warn(`≈1444≈0ZZa≈ composeCardinalityString(); key: ${element.key}`)
   ...
   if (displayParamObject.develLogKeyArray.includes(element.key) === true) {
       console.warn(`≈1488≈0ZZb≈ composeCardinalityString(): key: ${element.key}`)
    ...
    // ø </DO prependCardinalityItem>
    if (displayParamObject.develLogKeyArray.includes(element.key) === true) {
        console.warn(`≈1559≈0ZZc≈ composeCardinalityString(): [AFTER switch] key: ${element.key}`)
    ...
    if (displayParamObject.develLogKeyArray.includes(element.key)) {
        // console.warn(`≈1038≈ testCollectionReportStringPrep <≈1568≈ForEach => composeCardinalityString()>`)
        console.warn(`≈1568≈ testCollectionAppend(): key: ${element.key}`)
        let trimmedElement = JSON.parse(JSON.stringify(element))
        delete trimmedElement.notes
        // delete trimmedElement.response              //NA
        // delete trimmedElement.keyCountA             //NA
...
function composeReportStringDisplay(element = {}, displayParamObject = {}) {
    ...
    if (displayParamObject.develLogKeyArray.includes(element.key)) {
        console.warn(`≈1538≈0ZZZ≈ composeReportStringDisplay(): key: ${element.key}`)
        let trimmedElement = JSON.parse(JSON.stringify(element))
        delete trimmedElement.notes
        delete trimmedElement.response
        delete trimmedElement.keyCountA
...
async function logTestUnit(testUnit = {}, displayParamObject = {}, keyShowArray = [], lineObject = {}){
    ...
    // let proceed = false
    let proceed = true
    if (proceed && displayParamObject.develLogKeyArray.includes(testUnit.key)) {
        console.warn(`testUnit.key: ${testUnit.key}`)
        console.warn(`displayParamObject.develLogKeyArray: [${displayParamObject.develLogKeyArray}]`)
        lineObject.logLine ??= ''
