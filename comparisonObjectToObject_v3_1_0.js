...
≈ => searchable approximate line numbers (in console log)
    ...
    ≈789≈X!≈ approximate line number 789 console log indicates code that needs attention
    ≈789≈X!≈ same as above, but commented out (won't be found with RegEx)
...
async function displayPrePstWix(preWixRecord = {}, pstWixRecord = {}) {
    ...
    // console.warn(`≈189≈X≈ paramObject: AFTER: 'overrideWithGoldDefaults(paramObjectLocal)' [check Title & Sub-Title]`)
...
async function overrideWithGoldDefaults(paramObject = {}) {
    ...
    let supportedCardinalityTypeArray = ['123', 'ABC', 'abc', 'NONE','PRESET']//,'IIIIII','iiiiii']
    let supportedCardinalityPunctArray = ['.', ')', ':', '=>', 'NONE']//[almost anything]
    ...
    paramObjectDefault.displayParamObject.develLogKeyArray = ['Attribute','title','courseKey','waitList','waitlistCount']
    ...
    // ø ø <OVDERRIDE LOOP :paramObjectDefault.displayParamObject[assertion] >
    paramObjectDefault.displayParamObject.HEADER.cardinalityType = 'NONE'
    // ø ø </OVDERRIDE LOOP :paramObjectDefault.displayParamObject[assertion] >
    // ø ø <AGGREGATE:displayParamObject by assertion >
    paramObjectDefault.displayParamObject.COLUMNHEADER.defaultCardinalString = '#'
    paramObjectDefault.displayParamObject.anyCardinalityType = false //any Not 'NONE'
    paramObjectDefault.displayParamObject.cardinalityTypeByAssertionArray = []
    for (const assertion of supportedAssertionArray) {
        paramObjectDefault.displayParamObject.anyCardinalityType = paramObjectDefault.displayParamObject[assertion].cardinalityType !== 'NONE' ? true : paramObjectDefault.displayParamObject.anyCardinalityType
        paramObjectDefault.displayParamObject.cardinalityTypeByAssertionArray.push(paramObjectDefault.displayParamObject[assertion].cardinalityType)
        // paramObjectDefault.displayParamObject[assertion] = {}
        // paramObjectDefault.displayParamObject[assertion].minPaddingExtra = 1
        // paramObjectDefault.displayParamObject[assertion].cardinalityType = paramObjectDefault.displayParamObject.defaultCardinalityType
        // paramObjectDefault.displayParamObject[assertion].cardinalityPunct = paramObjectDefault.displayParamObject.defaultCardinalityPunct
    ...
    // console.warn(`paramObject: ≈367≈X≈ [after await][inside overrideWithGoldDefaults()]`)
    // console.dir(paramObject)
...
async function comparisonWixToWixRecords(wixRecordA = {}, wixRecordB = {}, paramObject = {}) {
    ...
    // console.warn(`≈416≈X≈ testCollection: [${testCollection}] \n[instantiated as empty array]`)
    ...
    // ø <COLUMNHEADER CARDINALITY>
    if(testCollection.length === 1){
        // console.warn(`testCollection.length === 1: ${testCollection.length === 1}`)
        if(paramObject.displayParamObject.anyCardinalityType){
            // console.warn(`≈463≈X≈ paramObject.displayParamObject.anyCardinalityType: ${paramObject.displayParamObject.anyCardinalityType}`)
            testCollection[0].reportCardinalString = '#|'
            testCollection[0].reportCardinalStringNOTE = 'via anyCardinalityType'
        }
        
    }else{
        console.error(`testCollection.length !== 1 => testCollection.length === ${testCollection.length}`)
    }
    // ø </COLUMNHEADER CARDINALITY>
    let keyShowArray = []
    let lineObject = {}
    ...
    await logTestUnit(testCollection[0], paramObject.displayParamObject, keyShowArray, lineObject, 'HIDE')
    ...
    // console.warn(`≈489≈X≈ testCollection <≈741≈ForEach>[instantiate by keyArrayKeyArray]`)
    ...
    // console.warn(`≈498≈X≈ testCollection </≈741≈ForEach>[instantiate by keyArrayKeyArray]`)
    
    // console.warn(`≈500≈X≈ testCollection <≈629≈ForEach>[instantiate by ObjectA-ObjectB]`)
    byTestCollectionObjectAObjectB(wixRecordA, wixRecordB, testCollection, paramObject)
    // console.warn(`≈502≈X≈ testCollection </≈629≈ForEach>[instantiate by ObjectA-ObjectB]`)
    ...
    // ¡ ===========        `response.testCollection` changed to `testCollection`       ===========
    // console.warn(`≈466≈X≈ paramObject => paramObject.displayParamObject: [console.warn() below]`)
    ...
    //     return substetArray.includes(testUnit.key)
    // })
    // console.warn(`reportRowDisplaySubSetFilter: ≈484≈X≈ AFTER: testCollectionReportComposePaddedString: [subset of item array below]`)
    // console.dir(reportRowDisplaySubSetFilter)
    // ø </reportRowDisplaySubSetFilter: AFTER: testCollectionReportComposePaddedString> 
    ...
    // })
    // console.warn(`reportRowDisplayMap: ≈501≈X≈ NOTE: reportStringDisplay: [array below]`)
    ...
    // console.warn(`≈510≈X≈ just after calling 'testCollectionReportDisplay'`)
    // console.warn(`≈511≈X≈ done... other than perhaps 'echoing' the report`)
    // console.warn(`≈512≈X≈ Force return`)
    // return
    ...
    // ¡ ===========        `response.testCollection` changed to `testCollection`       ===========
    // console.warn(`≈599≈X≈paramObject.displayParamObject.columnAlignmentArray: ${paramObject.displayParamObject.squaredPadding}`)
...
function byTestCollectionObjectAObjectB(objectA = {}, objectB = {}, testCollection = [], paramObject = {}) {
    ...
    // console.warn(`≈524≈X≈ objectB.regionKey: ${objectB.regionKey}`)
    // let responseObject = {}
    ...
    for (let index = 0; index < testCollection.length; index++) {
        ...
        let keyShowArray = []
        let lineObject = {}
        lineObject.logLine = '≈629≈'
        lineObject.logScriptName = 'byTestCollectionObjectAObjectB'
        lineObject.logLoopLine = ''//''
        lineObject.logLoopScirptName = ''//'testCollection'
        logTestUnit(testUnit, paramObject.displayParamObject, keyShowArray, lineObject, 'HIDE')
        
        if (paramObject.displayParamObject.develLogKeyArray.includes(testUnit.key)) {
            // console.warn(`≈632≈X!≈ ¿KEEP & 'HIDE'? byTestUnitObjectAObjectB(): [After SWITCH-ed Singletons] key: ${testUnit.key}`)
...
async function testCollectionAppend(testCollection = [], assertion = 'OTHER', keyArray = [], paramObject = {}) {
    for (let index = 0; index < keyArray.length; index++) {
        ...
        let keyShowArray = []
        let lineObject = {}
        lineObject.logLine = '≈741≈'
        lineObject.logScriptName = 'testCollectionAppend'
        lineObject.logLoopLine = ''//''
        lineObject.logLoopScirptName = ''//'testCollection'
        logTestUnit(testUnitThis, paramObject.displayParamObject, keyShowArray, lineObject, 'HIDE')
...
async function testCollectionReportStringPrep(testCollection = [], displayParamObject = {}) {
    let maxColCount = 0
    // console.warn(`<≈1075≈X≈> testCollectionReportStringPrep <≈ZZZ≈ForEach => composeReportStringDisplay()>`)
    for (let index = 0; index < testCollection.length; index++) {
        const element = testCollection[index];
        displayParamObject.columnMaxLengthArray = []
        // ø TEST_COLLECTION_REPORT_COMPOSE_CARNDINALITY_STRING [singleton] testUnit.reportCardinalString [DATA]
        // console.warn(`≈1038≈X≈ testCollectionReportStringPrep <≈1568≈ForEach => composeCardinalityString()>`)
        ...
        // KEEP && 'HIDE'
        let keyShowArray_1082LINE = []
        let lineObject_1082LINE = {}
        lineObject_1082LINE.logLine = '≈1538≈'
        lineObject_1082LINE.logScriptName = 'composeReportStringDisplay'
        lineObject_1082LINE.logLoopLine = ''//''
        lineObject_1082LINE.logLoopScirptName = ''//'testCollection'
        logTestUnit(element, displayParamObject, keyShowArray_1082LINE, lineObject_1082LINE, 'HIDE')

        if(['AZttribute','wZaitList'].includes(element.key)){
            console.warn(`<≈1109≈> element: [element.key: ${element.key}]`)
            ...
            console.warn(`</≈1117≈> element: [element.key: ${element.key}]`)
            console.dir(element)
            console.warn(`≈1119≈ displayParamObject: [just waitList => 'INCREMET']`)
        ...
        if(element.key === 'AZttribute'){
            console.warn(`<≈1127≈> element: [element.key: ${element.key}]`)
            console.warn(`element.assertion: ${element.assertion}`)
            console.warn(`element.assertionCardinal: ${element.assertionCardinal}`)
            console.warn(`element.reportCardinalString: '${element.reportCardinalString}'`)
            console.warn(`element.reportCardinalStringNOTE: '${element.reportCardinalStringNOTE}'`)
            ...
            console.warn(`element.reportStringDisplay: '${element.reportStringDisplay}'`)
            console.warn(`element.reportStringColCount: '${element.reportStringColCount}'`)
            console.warn(`</≈1135≈> element: [element.key: ${element.key}]`)
            console.dir(element)
            console.warn(`≈1137≈ displayParamObject: [just waitList => 'INCREMET']`)
            console.dir(displayParamObject)
            console.dir(displayParamObject.INCREMENT.columnAlignmentArray)
            // console.dir(displayParamObject.maxColPaddingArray)// = maxColPaddingArray)
            // console.dir(displayParamObject.minColPaddingExtraArray)// = minColPaddingExtraArray
        ...
        lineObject.logLoopLine = ''//''
        lineObject.logLoopScirptName = ''//'testCollection'
        logTestUnit(element, displayParamObject, keyShowArray, lineObject, 'HIDE')
    }
    // console.warn(`</≈1058≈X≈> testCollectionReportStringPrep </≈ZZZ≈ForEach => composeReportStringDisplay()>`)
}
...
async function testCollectionReportPaddingPrep(testCollection = [], displayParamObject = {}) {
    ...
    */
    // ø <PADDING_PREP-Logs>
    // console.warn(`≈1187≈X≈ displayParamObject. [testCollectionReportPaddingPrep]`)
    // console.warn(`≈1188≈X≈ displayParamObject.maxColPaddingArray [array below]`)
    // console.dir(displayParamObject.maxColPaddingArray)// = maxColPaddingArray)
    // console.warn(`≈1190≈X≈ displayParamObject.minColPaddingExtraArray [array below]`)
    // console.dir(displayParamObject.minColPaddingExtraArray)// = minColPaddingExtraArray
    // console.warn(`≈1192≈X≈ displayParamObject.columnAlignmentArray [array below]`)
    // console.dir(displayParamObject.columnAlignmentArray)// = columnAlignmentArray
    // ø </PADDING_PREP-Logs>
...
async function testCollectionReportDisplay(testCollection = [], displayParamObject = {}) {
    console.warn(`<----------- <entering 'testCollectionReportDisplay'> ----------->`)
    ...
    let paddingAppicableForAssertionArray = displayParamObject.paddingAppicableForAssertionArray ? displayParamObject.paddingAppicableForAssertionArray : []
    // console.warn(`≈1086≈X≈ paddingAppicableForAssertionArray: [${paddingAppicableForAssertionArray}]`)
    // console.dir(testCollection)
    ...
    // })
    // console.warn(`≈1180≈X≈ subHeaderWrappedArrayCentered: `)
    ...
    console.warn(`Force return: ≈1418≈ from 'testCollectionReportDisplay'`)
...
function deliverRowByMechanism(element = {}, mechanism = 'MECHANISM') {
    ...
    switch (mechanism) {
        ...
        default:
            // console.warn(`≈1166≈X≈ with 'defaultMechanism' this shoud never be reached, `)
            // console.warn(`≈1167≈X≈ the default must not be supported, no solution, just fix it.`)
...
function composeCardinalityString(element = {}, displayParamObject = {}) {
    ...
    displayParamObject.COLUMNHEADER.cardinalityType = 'PRESET'
    ...
    if(element.assertion === 'COLUMNHEADER'){
        // console.warn(`≈1621≈X≈ displayParamObject: [for devel override locally]`)
        // console.dir(displayParamObject)
        let keyShowArray = []
        ...
        // console.warn(`≈1631≈X≈ keyArray: [${keyArray}]`)
...
//        console.warn(`≈1669≈X≈ 0ZZb≈X!≈ composeCardinalityString(): key: ${element.key}`)
//        console.warn(`≈1670≈X≈ PARAMCardinalityType: ${PARAMCardinalityType}`)
//        console.warn(`≈1671≈X≈ PARAMCardinalityPunct: ${PARAMCardinalityPunct}`)
//        console.warn(`≈1672≈X≈ PARAMCardinalityAlign: ${PARAMCardinalityAlign}`)
//     }
    // ø --
    // return
    // ø --
    let elementCardinalityType = displayParamObject[element.assertion].cardinalityType
    // console.warn(`≈1678≈X≈ elementCardinalityType: ${elementCardinalityType}`)
    // ø --
    ...
    // ø </DO prependCardinalityItem>
    // if (displayParamObject.develLogKeyArray.includes(element.key) === true) {
    //     console.warn(`≈1703X≈ ≈0ZZc≈X!≈ composeCardinalityString(): [AFTER switch] key: ${element.key}`)
    //     console.warn(`≈1704≈X≈ [AFTER switch] element.reportCardinalStringNOTE: ${element.reportCardinalStringNOTE}`)
    //     console.warn(`≈1705≈X≈ PARAMCardinalityPunct: ${PARAMCardinalityPunct}`)
    //     console.warn(`≈1706≈X≈ PARAMCardinalityAlign: ${PARAMCardinalityAlign}`)
    // }
    // ø --
    // ø --
    // if (displayParamObject.develLogKeyArray.includes(element.key)) {
    //     console.warn(`≈1038≈X≈ testCollectionReportStringPrep <≈1568≈ForEach => composeCardinalityString()>`)
    //     console.warn(`≈1712≈X≈ testCollectionAppend(): key: ${element.key}`)
...
function composeReportStringDisplay(element = {}, displayParamObject = {}) {
    ...
    if(['wZaitList'].includes(element.key)){
        console.warn(`≈1761≈ element.reportRowDisplay: '${element.reportRowDisplay}'`)
        console.warn(`≈1762≈ reportRowDisplayArray: [${reportRowDisplayArray}]`)
        console.warn(`≈1763≈ indexArray: [${indexArray}]`)
    }
    let eqBuffer = '===================================================================================================='
    ...
    if (element.assertion === 'COLUMNHEADER') {
        ...
        if(element.reportCardinalString){
            if(element.assertion === 'COLUMNHEADER'){
                // console.warn(`≈1804≈X≈ element.assertion: ${element.assertion}`)
            }
...
function prependCardinalityToColumnHeader(testCollection = [],cardinalidyHeaderString = '#'){
    let columnHeaderCollection = testCollection.filter((row) => {
        return row.assertion === 'COLUMNHEADER'
    })
}


// ø ========== <DEVEL_AREA> ==========
async function logTestUnit(testUnit = {}, displayParamObject = {}, keyShowArray = [], lineObject = {}, option = 'SHOW'){
    if(option === 'HIDE'){
        return
    }
    ...
    if (displayParamObject.develLogKeyArray.includes(testUnit.key)) {
        ...
        console.dir(trimmedElement)
        // console.warn(`≈X≈741≈629≈ proceed: ${proceed}`)
        console.warn(`≈X≈741≈629≈1052≈ testUnit.key: ${testUnit.key}`)
